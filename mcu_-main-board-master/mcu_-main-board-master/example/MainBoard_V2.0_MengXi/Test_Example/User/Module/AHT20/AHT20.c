...
{
	uint8_t tmp = 0;
    HAL_I2C_Master_Receive(AHT20_I2C, AHT20_SLAVE_ADDRESS, &tmp, 1, 0xFFFF);
	return tmp;
}
...
static void AHT20_IcInitCmd(void)
{
	...
	HAL_I2C_Master_Transmit(AHT20_I2C, AHT20_SLAVE_ADDRESS, &tmp, 1, 0xFFFF);
}
...
static void AHT20_TrigMeasureCmd(void)
{
	...
	HAL_I2C_Master_Transmit(AHT20_I2C, AHT20_SLAVE_ADDRESS, tmp, 3, 0xFFFF);
}
...
static void AHT20_Register_Reset(uint8_t addr)
{
  ...
  iic_tx[0] = addr;
  HAL_I2C_Master_Transmit(AHT20_I2C, AHT20_SLAVE_ADDRESS, iic_tx, 3, 0xFFFF);
  HAL_Delay(5);
  HAL_I2C_Master_Receive(AHT20_I2C, AHT20_SLAVE_ADDRESS, iic_rx, 3, 0xFFFF);
  HAL_Delay(10);
  iic_tx[0] = 0xB0 | addr;
  iic_tx[1] = iic_rx[1];
  iic_tx[2] = iic_rx[2];
...
uint8_t AHT20_ReadHT(uint32_t *HT)
{
	...
	if(cnt)//设备闲，可以读温湿度数据
	{
		HAL_Delay(5);
		// 读温湿度数据
        HAL_I2C_Master_Receive(AHT20_I2C, AHT20_SLAVE_ADDRESS, tmp, 6, 0XFFFF);
